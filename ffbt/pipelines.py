...
class FffbtPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('ffbt.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `ffbt`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM ffbt WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO ffbt (date, title, link, content)"
